<x-app-layout>
    <div x-data="eventForm()" class="min-h-screen font-sans text-slate-800 relative z-10 w-full animate-fade-in pb-16">
        ...
        <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data" class="max-w-5xl mx-auto relative z-10 p-4 lg:p-0">
            ...
            <header class="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-10">
                <div>
                    <nav class="flex text-[11px] font-bold text-slate-500 mb-3 gap-3 uppercase tracking-widest items-center">
                        ...
                        <span class="text-indigo-600">Edit Event</span>
                    </nav>
                    <h1 class="text-5xl font-extrabold tracking-tight text-slate-900">Edit <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-blue-600 to-purple-600">Details</span></h1>
                </div>
                <div class="flex gap-4 w-full md:w-auto">
                    <!-- Duplicate Button ported from original edit page -->
                    <button type="submit" form="eventDuplicateForm" class="flex-1 md:flex-none text-center px-6 py-3.5 bg-white border border-slate-200 rounded-2xl font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-50 hover:border-slate-300 hover:shadow-sm transition-all duration-300 flex items-center justify-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2"></path></svg>
                        Duplicate
                    </button>
                    <a href="{{ route('events.show', $event) }}" class="flex-1 md:flex-none text-center px-6 py-3.5 bg-white border border-slate-200 rounded-2xl font-bold text-indigo-600 hover:bg-slate-50 hover:border-slate-300 hover:shadow-sm transition-all duration-300">View Page</a>
                    <button type="submit" class="flex-1 md:flex-none text-center px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold shadow-[0_10px_20px_-10px_rgba(37,99,235,0.5)] hover:shadow-[0_20px_40px_-10px_rgba(37,99,235,0.6)] transition-all duration-300 flex items-center justify-center gap-2 group border border-blue-700">
            ...
            <div class="space-y-8">
                ...
                <section class="bg-white/60 backdrop-blur-3xl border border-white/80 rounded-[2.5rem] p-8 lg:p-12 shadow-[0_20px_80px_-20px_rgba(0,0,0,0.05)] relative overflow-hidden group">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                        <div class="col-span-1 md:col-span-2 space-y-2">
                            ...
                            <div class="relative">
                                <input type="text" name="title" id="title" value="{{ old('title', $event->title) }}" placeholder="e.g. Annual Tech Fest 2026" required class="w-full bg-white/80 backdrop-blur-md border border-slate-200/60 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] rounded-2xl px-6 py-4 focus:ring-4 focus:ring-indigo-500/15 focus:border-indigo-400 focus:bg-white transition-all outline-none text-xl font-bold text-slate-800 placeholder-slate-400">
                            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="datetime-local" name="date" id="date" value="{{ old('date', $event->date ? $event->date->format('Y-m-d\TH:i') : '') }}" required class="w-full bg-white/80 backdrop-blur-md border border-slate-200/60 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] rounded-2xl px-6 py-4 focus:ring-4 focus:ring-indigo-500/15 focus:border-indigo-400 focus:bg-white transition-all outline-none text-slate-700 font-medium font-mono">
                ...
                <section class="bg-white/60 backdrop-blur-3xl border border-white/80 rounded-[2.5rem] p-8 lg:p-12 shadow-[0_20px_80px_-20px_rgba(0,0,0,0.05)] relative overflow-hidden group">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                        ...
                        <div class="col-span-1 md:col-span-2 space-y-3" x-data="{
                            ...
                            selected: {{ json_encode(old('organizing_departments', $event->organizing_departments ?? [])) }},
                            toggleAll() {
                        ...
                        <div class="col-span-1 md:col-span-2 space-y-4" x-data="{ openToAll: '{{ old('is_open_to_all', $event->is_open_to_all) ? '1' : '0' }}' }">
                            ...
                            <div x-show="openToAll === '0'" x-transition.opacity.duration.300ms style="display: none;">
                                <div class="mt-4 p-5 bg-white/60 border border-slate-200 rounded-2xl shadow-inner" x-data="{
                                    allSelected: false,
                                    departments: {{ json_encode($departments->pluck('name')) }},
                                    selected: {{ json_encode(old('eligible_departments', $event->eligible_departments ?? [])) }},
                                    toggleAll() {
                                        this.selected = this.allSelected ? [...this.departments] : [];
                                    },
                                    updateAllSelected() {
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="ticket_price" id="ticket_price" value="{{ old('ticket_price', $event->ticket_price) }}" min="0" step="0.01" class="w-full pl-12 bg-white/70 backdrop-blur-md border border-white shadow-inner rounded-2xl pr-6 py-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-300 focus:bg-white transition-all outline-none text-slate-700 font-bold font-mono">
                            </div>
                        ...
                        <div class="space-y-2">
                            <label class="block text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Published Status</label>
                            <label class="flex items-center cursor-pointer mt-2">
                                <div class="relative">
                                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', $event->is_published) ? 'checked' : '' }} class="sr-only" x-model="isPublished">
                                    <div class="block w-14 h-8 rounded-full shadow-inner transition-colors duration-300" :class="isPublished ? 'bg-blue-600' : 'bg-slate-200'"></div>
                                    <div class="dot absolute left-1 top-1 w-6 h-6 bg-white rounded-full transition-transform duration-300 shadow-sm" :class="isPublished ? 'translate-x-6' : ''"></div>
                                </div>
                                <span class="ml-4 text-sm font-bold text-slate-700" x-text="isPublished ? 'Live' : 'Draft'"></span>
                            </label>
                        </div>
                        ...
                        <div class="col-span-1 md:col-span-2 space-y-2">
                            ...
                            <textarea name="description" id="description" rows="5" required placeholder="Outline the event protocol, agenda, and requirements..." class="w-full bg-white/70 backdrop-blur-md border border-white shadow-inner rounded-2xl px-6 py-5 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-300 focus:bg-white transition-all outline-none text-slate-700 font-medium placeholder-slate-400 resize-y min-h-[150px] leading-relaxed">{{ old('description', $event->description) }}</textarea>
                ...
                @php
                    ...
                    $volunteerIds = $event->team->where('pivot.role', 'volunteer')->pluck('id')->map(fn($id) => (string)$id)->toArray();
                @endphp
                <section class="bg-white/60 backdrop-blur-3xl border border-white/80 rounded-[2.5rem] p-8 lg:p-12 shadow-[0_20px_80px_-20px_rgba(0,0,0,0.05)] relative z-40 overflow-visible group"
                    x-data="{ 
                        selectedHeadFacultyId: '{{ $headFacultyId }}',
                        selectedStaffIds: {{ json_encode($staffIds) }},
                        selectedVolunteerIds: {{ json_encode($volunteerIds) }},
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="relative" x-data="{ open: false, search: '' }">
                                ...
                                <div x-show="open" x-transition.opacity.duration.200ms style="display: none;" class="absolute z-[60] w-full mt-2 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-100/50 overflow-hidden">
                                    <div class="p-3 border-b border-slate-100/50 bg-slate-50/50">
                                        <div class="relative">
                                            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                                            <input type="text" x-model="search" placeholder="Search faculty by name..." class="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 outline-none transition-all placeholder-slate-400 text-slate-700 shadow-inner">
                                        </div>
                                    </div>
            ...
            <div class="fixed bottom-0 left-0 right-0 z-50 p-4 pointer-events-none pb-8">
                <div class="max-w-5xl mx-auto flex justify-end">
                    <div class="bg-white/40 backdrop-blur-2xl p-2 rounded-[2rem] shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] border border-white/60 flex items-center gap-3 pointer-events-auto transform transition-all duration-500 hover:shadow-[0_30px_60px_-15px_rgba(79,70,229,0.2)] hover:border-indigo-200 group/bar">
                        <button type="button" onclick="window.history.back()" class="px-6 py-4 text-slate-500 hover:text-slate-800 font-bold rounded-2xl transition-colors hover:bg-white/60 flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                            Cancel
                        </button>
                        <button type="submit" class="relative overflow-hidden group/btn px-10 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-[1.5rem] font-bold shadow-[0_10px_20px_-5px_rgba(37,99,235,0.3)] transition-all duration-300 flex items-center gap-3 hover:shadow-[0_20px_40px_-10px_rgba(37,99,235,0.5)] transform hover:-translate-y-1 border border-blue-700">
                            <!-- Shimmer effect -->
                            ...
                            <span class="relative z-10 text-lg uppercase tracking-wider">Update Sequence</span>
                            <div class="relative z-10 w-8 h-8 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-md group-hover/btn:bg-white/30 transition-colors">
                                <svg class="w-4 h-4 group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                            </div>
                        </button>
                    </div>
                </div>
        ...
        <!-- Separate form for duplication logic -->
        <form id="eventDuplicateForm" action="{{ route('events.duplicate', $event) }}" method="POST" class="hidden">
            @csrf
        </form>
    </div>
